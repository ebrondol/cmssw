...
namespace helper {
  ...
  void ClusterStorer::addCluster(TrackingRecHitCollection &hits, size_t index)
  {
    ...
    if (hit_type == typeid(SiPixelRecHit)) {
      ...
      std::cout << "|   It is a VectorHit hit and it cannot be recorded because it does not have two recHit but just two clusters !!" << std::endl;
//      VectorHit &vhit = static_cast<VectorHit&>(newHit);
//      phase2ClusterRecords_.push_back(Phase2ClusterHitRecord(static_cast<VectorHit&>(newHit), hits, index));
    } else {
