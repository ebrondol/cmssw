...
class Phase2MeasurementConditionSet {
public:
  Phase2MeasurementConditionSet(const PixelClusterParameterEstimator *cpe) :
    theCPE(cpe) {}
...
public:
  ...
  Phase2MeasurementDetSet(const Phase2MeasurementConditionSet &cond) :
    conditionSet_(&cond),
    detSet_(cond.nDet()),
    empty_(cond.nDet(), true),
    activeThisEvent_(cond.nDet(), true)  {}

  const Phase2MeasurementConditionSet & conditions() const { return *conditionSet_; }
...
private:
  ...
  const Phase2MeasurementConditionSet *conditionSet_;
