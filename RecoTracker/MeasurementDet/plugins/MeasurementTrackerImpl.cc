...
{ 
  ...
  initPhase2MeasurementConditionSet(thePhase2Dets);
...
void MeasurementTrackerImpl::initPhase2MeasurementConditionSet(std::vector<TkPhase2MeasurementDet> & phase2Dets)
{
  // assume vector is full and ordered!
    ...
    if (gdu->isLeaf()) {
      if(subIsPixel) {
        if(!subIsOT) {
          ...
          LogDebug("MeasurementTracker") << " Pixel GeomDetUnit for OT " << (**gd).geographicalId().rawId();
          addPhase2Det(*gd);
        }
      } else {
      ...
      LogDebug("MeasurementTracker") << " Stack GeomDetUnit " << (**gd).geographicalId().rawId();
...
{
  try {
    thePhase2Dets.push_back(TkPhase2MeasurementDet( gd, thePhase2DetConditions ));
  }
  catch(MeasurementDetException& err){
    edm::LogError("MeasurementDet") << "Oops, got a MeasurementDetException: " << err.what() ;
  }
