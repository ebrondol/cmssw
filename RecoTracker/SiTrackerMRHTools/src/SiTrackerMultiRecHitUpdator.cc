...
//      if (cloned->isValid()) updatedcomponents.push_back(cloned);
  ...
  //ERICA: ierr will be set to 0 when inversion is successfull
  int ierr;
  ...
  for(std::vector<TransientTrackingRecHit::RecHitPointer>::iterator ihit = updatedcomponents.begin(); ihit != updatedcomponents.end(); ihit++) {
    ...
    //ERICA: the Det2 method will preserve the content of the Matrix and return true when the calculation is successfull
    double det;
