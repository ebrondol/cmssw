...
void TICLTrackstersValidation::bookHistograms(DQMStore::IBooker& ibook,
  ...
  static const char* particle_kind[] = {"photon", "electron", "muon", "neutral_pion", "charged_hadron", "neutral_hadron", "ambiguous", "unknown"};
  ...
  for (const auto& trackster_token : tracksterTokens_) {
    ...
    histo.id_prob_ = ibook.book1D("ID probability", "ID probability;category;Max ID probability", nCategory, 0, nCategory);
    histo.id_prob_1plusLC_ = ibook.book1D("ID probability "+onePlusLC[0], "ID probability "+onePlusLC[1]+";category;Max ID probability", nCategory, 0, nCategory);
