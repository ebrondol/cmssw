                                    ...
                                    edmNew::DetSetVector<Phase2TrackerCluster1D>& clustersRej)
{
  ...
  edmNew::DetSetVector<Phase2TrackerCluster1D>::const_iterator DSViter1;
  for( DSViter1 = ClustersPhase2Collection->begin() ; DSViter1 != ClustersPhase2Collection->end(); DSViter1++){

    unsigned int rawDetId1(DSViter1->detId());
    ...
    LogDebug("VectorHitBuilderAlgorithm") << "  DetId lower cluster : " << rawDetId1;
    LogDebug("VectorHitBuilderAlgorithm") << "  DetId upper cluster : " << detId2.rawId();

    const GeomDet* gd = theTkGeom->idToDet(detIdStack);
