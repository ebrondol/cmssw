...
string skipEvent = "";
std::string folderName = "/afs/cern.ch/work/e/ebrondol/GeomSLHC/CMSSW_6_2_SLHCDEV_X_2016-04-10-0000/src/RecoLocalTracker/SiPixelVectorHitBuilder/test/";
std::string geometry = "tilted";
std::string FileName1 = "cluster_validation_"+geometry+".root";
std::string FileName2 = "vh_validation_"+geometry+".root";
...
//  PlotComparisonClustersPixelStrip("RVsZ", true);
  ...
  PlotComparisonClustersPixelStripVHArray("RVsZ");

  FolderRootName2 = baseFolderRootName + "GlobalPositions/";
//  PlotComparisonClustersPixelStrip("YVsX", true);
  FolderRootName2 = baseFolderRootName + "Directions/";
  PlotComparisonClustersPixelStripVHArray("YVsX");
  return;
}
...
void PlotComparisonClustersPixelStripVHArray(const char* HistoName, bool debug = false ){
  ...
  if(HistoName=="RVsZ"){
    ...
    histo1strip -> GetYaxis() -> SetTitle("r [cm]");
    histo1strip -> GetXaxis() -> SetRangeUser(-300.0, 300);
    histo1strip -> GetXaxis() -> SetTitle("z [cm]");
  }
  if(HistoName=="YVsX"){
  ...
  while ((obj=next())) {
    if(debug)     cout << "Reading: " << obj->GetName() << endl;
