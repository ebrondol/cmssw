#include "RecoLocalTracker/SiPixelVectorHitBuilder/test/VectorHitsValidation.h"
...
void VectorHitsBuilderValidation::analyze(const edm::Event& event, const edm::EventSetup& eventSetup) {
    ...
    event.getByLabel(srcClu_, clusters);

    // Get the vector hits
    edm::Handle< VectorHitCollectionNew > vhs;
    event.getByLabel(srcVH_, vhs);
      ...
      for (edmNew::DetSet< VectorHit >::const_iterator vhIt = DSViter->begin(); vhIt != DSViter->end(); ++vhIt) {
        ...
        if( vhIt->isValid() ){
          ...
          if (layer == 1 || layer == 2 || layer == 3) {
           ...
           // Get the simTrack that pass into the first digi of the lower clusters
           // ERICA: it should be done also for the outer? don t think so.
           for (unsigned int i(0); i < (*(vhIt->lowerClusterRef().cluster_phase2())).size(); ++i) {
             ...
             unsigned int InnerChannel(PixelDigi::pixelToChannel( (*(vhIt->lowerClusterRef().cluster_phase2())).firstRow() + i, (*(vhIt->lowerClusterRef().cluster_phase2())).column()));
...
void VectorHitsBuilderValidation::printCluster(const GeomDetUnit* geomDetUnit, const OmniClusterRef cluster){
  ...
  MeasurementPoint mpClu((*cluster.cluster_phase2()).center(), (*cluster.cluster_phase2()).column() + 0.5);
